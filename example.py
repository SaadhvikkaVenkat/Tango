...
#########################################################################
                    ...
                    if ndist < 2:
                        ...
                        if loit in time_loit_db.keys():
                            ...
                            if loitering_time > 5 and loitering_time < 60 and loitering_time!=loitering_time_prev and current_time!=current_time_prev :
                                ...
                                if time_loit_db_prev >= time_loit_db[loit] or time_loit_db[loit] <= current_time_prev and time_loit_db[loit].hour==current_time_prev.hour and time_loit_db_prev!=datetime.datetime(1,1,1):
                                  xzxs="true"
                                  #print("\n\ntime_loit_db[loit]==",time_loit_db[loit]," time_loit_db_prev==",time_loit_db_prev," current_time_prev==", current_time_prev) 
                                  #sub_loitering_time=abs(loitering_time_prev-loitering_time)
                                  #print("loitering_time_prev==",loitering_time_prev)

                                print("loitering time----- ------",loitering_time,"  from    ",current_time,"  to  ",time_loit_db[loit])

                                tempp=list([emp_id1,emp_id2])
                                loit_emp.append(tempp)

                                if current_time.hour==time_loit_db[loit].hour and current_time.hour not in time_dict.keys():
                                    time_dict[current_time.hour]=loitering_time

                                    #print("new addition====== ==========",time_dict)

                                elif current_time.hour==time_loit_db[loit].hour and current_time.hour in time_dict.keys():
                                    time_dict[current_time.hour] =current_time.hour+ loitering_time
                                    print("existing value updation ^^^^^^^^  ^^^^^^^^^",time_dict) 

                                elif current_time.hour!=time_loit_db[loit].hour:
                                    if time_loit_db[loit].hour not in time_dict.keys():
                                        #if flag==1:
                                        time_dict[time_loit_db[loit].hour] = loitering_time
                                        #else:
                                         #   time_dict[time_loit_db[loit].hour] = current_time-time_loit_db[loit]
                                    else:
                                        #if flag==1:
                                        time_dict[current_time.hour]=loitering_time
                                        #else:
                                            #time_dict[current_time.hour]=current_time-time_loit_db[loit]
                                    print(" time taken in  2 time frames $$$$$$ $$$$$",time_dict)

                                print(emp_id1,"and",emp_id2,"loitered for",time_dict[current_time.hour],"minutes in ",current_time.hour,"hour")
            ...
            #for i,j in time_dict.items(): 
             #   print(emp_id1,"and",emp_id2,"loitered for",time_dict[current_time.hour],"minutes at ",current_time.hour)
                #print(emp_id1,"and",emp_id2,"loitered for",j,"minutes at ",i)
